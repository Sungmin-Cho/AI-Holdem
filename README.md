...
온라인 세션은 로비에서 세션을 열면 LAN 참가 링크(`http://<LAN IP>:8899/join?code=ABCD-EFGH`)가 생긴다. 기본은 암호화 없는 HTTP이며 포트포워딩·터널·인증서는 사용자 몫이다. 같은 NAT 뒤에서는 여러 참가자가 한 주소로 보일 수 있다. 멀티 세션은 앱으로만 재개하고, 롤백 전에는 게임을 종료하고 룸을 닫는다.
