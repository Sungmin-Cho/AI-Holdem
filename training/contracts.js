...
export function assertSnapshot(snapshot) {
  if (!snapshot || ![1, 2].includes(snapshot.schemaVersion) || typeof snapshot.decisionId !== 'string') {
  ...
  if (snapshot.schemaVersion === 2) {
    ...
    if (!legal || typeof legal !== 'object' || Array.isArray(legal)
      || legal.decisionId !== snapshot.decisionId
      || typeof legal.canCheck !== 'boolean'
      || typeof legal.canRaise !== 'boolean') {
      throw coded(ERRORS.SNAPSHOT_INVALID, 'authoritative legal evidence가 없습니다.');
    }
    for (const key of ['callAmount', 'minRaiseTo', 'maxRaiseTo']) {
      if (!Number.isInteger(legal[key]) || legal[key] < 0) {
        throw coded(ERRORS.SNAPSHOT_INVALID, `authoritative legal ${key}가 올바르지 않습니다.`);
      }
    }
    if (legal.canCheck !== (legal.callAmount === 0)
      || snapshot.toCall !== legal.callAmount
      || snapshot.minRaiseTo !== legal.minRaiseTo
      || snapshot.maxRaiseTo !== legal.maxRaiseTo) {
      throw coded(ERRORS.SNAPSHOT_INVALID, 'authoritative legal evidence와 snapshot 수치가 다릅니다.');
    }
  }
}
