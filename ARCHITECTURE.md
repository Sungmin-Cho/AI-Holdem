...
| `game/` | gitignore된 런타임 상태 — `loop-state.json`(딜러의 유일한 관찰 지점), `loop.lock.d/`(사이드카 수명 락), `state.json`(엔진 전용). |
...
- `game/state.json`은 `engine/state.js`의 락(`withMutation`)을 통해서만 바뀌어야 한다 — 사이드카·게시 도구도 상태를 직접 쓰지 않고 `engine/cli.js`를 자식 프로세스로 부른다.
...
- 종료 phase 체크포인트는 역행하면 안 된다: `playing → finalizing → review_generated → review_published → done`. `review_generated` 이후 재개는 `game/review.md`를 다시 만들지 않고, 먼저 기록해 둔 sha256으로 그 산출물을 재사용해야 한다.
...
| 딜러 세션(호스트 LLM) → 게임 루프 | `game/loop-state.json` 폴링, 사이드카 기동 | 핸드 안 진행에 관여, 상태 파일 직접 수정 |
...
- **관찰 지점**: `game/loop-state.json`이 딜러 세션의 유일한 폴링 대상이다 — phase·port·notices·metrics·halt가 여기 모인다. 다른 채널로 진행 상황을 흘리지 않는다.
- **로그**: 로그 파일을 여는 것은 사이드카뿐이다(`game/loop.log`, append). 사이드카가 띄우는 서버는 `stdio: 'ignore'`로 spawn되므로 자체 로그 파일을 갖지 않는다 — `game/server.log`는 서버를 손으로 띄울 때 쓰는 셸 리다이렉트일 뿐이다. 공용 로거는 없다.
