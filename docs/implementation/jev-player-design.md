...
응답 수용 조건: 객체/정확한 answer 이름과 type, 등록된 choice, 후보와 정확히 같은 probabilities 키 집합, 유한한 각 확률 [0,1], 합 1±1e-6 (모든 값이 0.01 단위이면 후보 수 × 0.005 + 1e-6의 반올림 상한 적용), confidence [0,1], 응답 model이 저장된 model과 일치, choice가 최대 확률(tolerance 1e-6; 동률이면 반환 choice 허용; 모든 값이 0.01 단위이면 반올림 봉투 0.01+1e-6 — 결정 규칙 v2 게이트에서 관측). usage는 있으면 비음수 safe integer로 검증하고 없으면 unknown이다. 분포·confidence도 공식 응답 계약의 일부로 엄격 검증한다. 합법 choice라도 필수 진단 필드가 손상되면 중단하는 가용성 대가를 의도적으로 수용한다. 이를 추후 완화하려면 별도 설계 변경으로 다룬다. 형식 불량은 `JEV_INVALID_RESPONSE`, 모델 불일치는 `JEV_MODEL_MISMATCH`; 응답 확률 자체를 조용히 고치지 않는다.
...
- **선택 규칙 `class-sample-v1`**(`selectJevAction`): 후보 확률을 행동 클래스(fold/check/call/raise)별로 합산하고, 질량 0.05 미만 클래스를 뺀 나머지를 합 1로 맞춘 뒤 고정 순서 `[fold, check, call, raise]`로 추첨한다. 레이즈가 뽑히면 레이즈 후보를 금액 오름차순으로 놓고 누적 확률이 레이즈 질량의 절반에 처음 닿는 사이즈(확률 가중 중앙값)를 고른다. 추첨값은 응답 전에 `(sessionToken, decisionId, generation)`에서 sha256으로 파생하므로 원격 응답이 추첨을 조종할 수 없고 loop-state 기록만으로 재계산할 수 있다. `sessionToken`은 참가자에게 해시(`gameEpoch`)로만 전달되는 비밀값이라 참가자가 AI의 추첨을 미리 계산할 수 없다(공개값만 쓰면 `gameEpoch`·`decisionId`로 예측 가능 — 구현 리뷰에서 발견). 재시도(세대 +1)는 새 추첨이다. 선택 결과 하나가 pending `proposedAction`과 엔진 `step` 양쪽에 쓰인다. 후보가 하나면 HTTP도 선택도 없다.
...
- **choice 반올림 봉투**(2026-09-23 실측): 실제 API가 0.01 단위 확률에서 최댓값(0.28)보다 0.01 낮은 후보(0.27)를 `choice`로 돌려준 사례가 약 460응답 중 1건 있었다. 반올림 전 값으로는 일관되므로, 모든 값이 0.01 단위일 때만 choice가 최댓값에서 0.01 이내면 수용한다(합 오차 보정과 같은 근거). v2는 API 라벨을 실행하지 않고 증거로만 기록하므로 실행 행동에는 영향이 없다. 그 외 정밀도의 1e-6 검증과 나머지 계약은 그대로다.
