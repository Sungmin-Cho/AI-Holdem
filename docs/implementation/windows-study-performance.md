...
Local POSIX lifecycle and security/recovery tests: 55 passed. Combined local integration before final test-fixture hardening: 2,308 passed, zero failed (803,974 ms); final exact counts are recorded in PR #186. Local benchmark smoke passed; POSIX has no PowerShell proof calls and is not Windows performance evidence. Windows paired results and full Node 20/22 Linux/Windows gates must be recorded before accepting the optimization.
...
Final Windows CI exposed a pre-existing repair-fixture assumption: a 50 ms synthetic decision budget can expire before repair, so absence of a repair-failure log is valid. The semantic fixtures now control their monotonic clock, explicitly cover both remaining and exhausted budget, assert one attempt and byte-preserved engine state, and enable the successful-repair test on Windows instead of its obsolete 25 ms skip. Production budgets are unchanged. Separate real-clock/real-child regressions retain timer and termination coverage.
