...
요청에는 행동하는 AI의 자기 패와 공개 테이블 정보만 좌석 별칭으로 보냅니다. 다른 좌석의 비공개 패, 참가자 이름·ID, 채팅, 의도 메모는 제외합니다. JEV는 합법 액션·금액 후보에서 선택합니다. 요청 실패 시 로컬 정책으로 자동 전환하지 않고 명시적인 복구를 기다립니다. 코치는 기존 LLM 연동 또는 사실 기반 피드백을 사용합니다.
