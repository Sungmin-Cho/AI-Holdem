...
새 `--store-dir` 게임은 cash-training·AI 5명·100BB·20핸드·policy v2·showdownPolicy=open·replayReveal=all이 기본이다. 옵션 `--showdown-policy open|standard`, `--replay-reveal all|showdown`. 로그 탭의 복기 뷰와 액션 바의 의도 메모를 쓴다. 명시한 `--opponent-runtime llm`, `--mode tournament`, `--mirror-self`·`--exploit-self`, AI 수·스택·블라인드·핸드 수를 보존한다. mode 없는 `--stack`/`--level-every`는 기존 토너먼트 설정이며, resume과 legacy `--game-dir`에는 새 기본값을 적용하지 않는다. policy 게임은 플레이어 LLM 없이 실행하고 상위 모델만 코치·리뷰 용도로 검사한다. 적격 상위 모델이 없으면 LLM 설명 불가를 알리고 사실 기반 기계 피드백을 남긴다.
