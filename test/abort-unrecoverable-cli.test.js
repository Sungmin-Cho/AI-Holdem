...
test('#197 real legacy CLI aborts invalid recovery with no provider CLI available and no late file write',{timeout:TIMEOUT},async()=>{
  ...
  const result=await run(process.execPath,args,{env:{...process.env,PATH:''},timeout:TIMEOUT-1000,maxBuffer:1024*1024});
  ...
  const again=await run(process.execPath,[path.resolve('tools/game-loop.js'),'--game-dir',root,'--resume'],{env:{...process.env,PATH:''},timeout:TIMEOUT-1000});
