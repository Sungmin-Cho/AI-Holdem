...
public static class OwnedJob {
 ...
 public static int Run(string command,string[] args,string cwd) {
   ...
   if(String.IsNullOrEmpty(command)||String.IsNullOrEmpty(cwd)||args==null||args.Length>1024) throw new Exception("WINDOWS_OWNED_PAYLOAD_INVALID");
...
'@
  $code = [OwnedJob]::Run([string]$spec.command, [string[]]$spec.args, [string]$spec.cwd)
  exit $code
} catch {
