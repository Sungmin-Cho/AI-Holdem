...
test('S8 full: default 20-hand production session records support then study remains usable and reusable', { timeout: 240000 }, async (t) => {
  ...
  t.after(async () => {
    ...
    for (const cleanup of [
      () => loop?.requestStop(),
      () => nextCli ? cleanupCli(nextCli, nextGameDir) : undefined,
      () => cleanupCli(initialCli, null),
      () => stopOwnedStudy(storeDir),
  ...
  const driver = (async () => {
    ...
    while (!settled && Date.now() < deadline) {
      ...
      const lock = JSON.parse(fs.readFileSync(path.join(gameDir, 'lock.json')));
      let snapshot;
      try { snapshot = await relayRequest(lock, '/api/snapshot'); }
      catch (error) { if (settled) break; throw error; }
